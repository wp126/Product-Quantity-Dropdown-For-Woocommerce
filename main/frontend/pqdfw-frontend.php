...
function pqdfw_custom_quantity_field_archive( $args = array(), $product = null, $echo = true ) {
	global $pqdfw_comman;
    if ( is_null( $product ) ) {
        	...
        	<div class = "quantity_drop_down"> 
  				...
  				<div class="quantity">
						<select class="qty_select" id="ss" name="<?php echo esc_attr($defaults['input_name']);?>" onchange="extendons_selectbox();">
							<?php echo _e($options,"product-quantity-dropdown-for-woocommerce"); ?>
						</select>
					</div>				          						
			...
			$var2 = 'false';		   		
		   	if(!empty(get_option('pqdfw_select2')) || !empty(get_option('pqdfw_cats_select2')) || !empty(get_option('pqdfw_tags_select2')) ){
		   		...
		   		if(!empty(get_option('pqdfw_select2'))){
		   			if(in_array(get_the_id() , get_option('pqdfw_select2'))){
		   				$var2 = 'true';
		   			}
		   		}
...
function PQDFW_custom_css(){
	...
	if( !empty(get_option('pqdfw_select2')) && in_array(get_the_id() , get_option('pqdfw_select2')) ) {
		if(in_array(get_the_id() , get_option('pqdfw_select2'))){
            ?>
            <style type="text/css">
                .quantity>.text {
...
global $pqdfw_comman;
	if($pqdfw_comman['enable_plugin'] == "yes" ){
		if($pqdfw_comman['quantity_position'] == 'after_addtocart'){
			add_action( 'woocommerce_after_add_to_cart_button','pqdfw_custom_quantity_field_archive');
			add_action( 'woocommerce_after_shop_loop_item', 'pqdfw_custom_quantity_field_shop_loop', 10 );
		...
		}elseif($pqdfw_comman['quantity_position'] == 'before_addtocart'){
			add_action( 'woocommerce_before_add_to_cart_form', 'pqdfw_custom_quantity_field_archive');
